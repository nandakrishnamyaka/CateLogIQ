...
def stream_viewer(request):
    ...
    # Azure configuration
    account_name = settings.AZURE_ACCOUNT_NAME
    parquet_path = f"abfss://silver@{account_name}.dfs.core.windows.net/silver_processed_logs.parquet/"
    ...
    try:
        ...
        # Read and cache Parquet data
        df = spark.read.parquet(parquet_path).cache()
        logger.debug("Parquet file loaded and cached")

        # Get all columns from Parquet
        available_columns = df.columns
        if not available_columns:
            logger.error("No columns found in Parquet file")
        ...
        cache_key_levels = f"stream_viewer_levels_{parquet_path}"
        ...
        logger.debug(f"Collected {len(data_list)} rows from Parquet")

        # Convert to pandas DataFrame for local filtering
        pandas_df = pd.DataFrame(data_list)
        if pandas_df.empty:
            logger.warning("No data collected from Parquet file")
        ...
        logger.error(f"Failed to query Parquet file: {str(e)}", exc_info=True)
